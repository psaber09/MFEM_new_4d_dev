  ...
  DenseMatrix &elmat )
{
   ...
   DenseMatrix curlshape(nd,dimc), curlshape_dFt(nd,dimc), M;
#else
   SkwGradshape.SetSize(nd,dimskw);
   SkwGradshape_dFt.SetSize(nd,dimskw);
#endif
   elmat.SetSize(nd);
   if (MQ) { M.SetSize(dimskw); }
   if (DQ) { D.SetSize(dimskw); }
   ...
   for (int i = 0; i < ir->GetNPoints(); i++)
   {
      ...
      if (dim ==4)
      {
       ...
       // SkwGrad SkwGrad operators
         //DenseMatrix J = Trans.Jacobian();
         //DenseMatrix Jtr(J); Jtr.Transpose();
        ...
        DenseMatrix invJtr(invJ); invJtr.Transpose();
         ...
         el.CalcSkwGradShape(ip, SkwGradshape);
         for (int dof=0; dof<nd; dof++)
         {
            tSh = 0.; trShTemp = 0.;
             
            //tSh(0,0) = SkwGradshape(dof,0);
            tSh(0,1) = SkwGradshape(dof,0);
            tSh(0,2) = SkwGradshape(dof,1);
            tSh(0,3) = SkwGradshape(dof,2);
             
             
            tSh(1,0) = -SkwGradshape(dof,0);
            //tSh(1,1) = SkwGradshape(dof,5);
            tSh(1,2) = SkwGradshape(dof,3);
            tSh(1,3) = SkwGradshape(dof,4);
             
             
            tSh(2,0) = -SkwGradshape(dof,1);
            tSh(2,1) = -SkwGradshape(dof,3);
            //tSh(2,2) = SkwGradshape(dof,10);
            tSh(2,3) = SkwGradshape(dof,5);
             
             
            tSh(3,0) = -SkwGradshape(dof,2);
            tSh(3,1) = -SkwGradshape(dof,4);
            tSh(3,2) = -SkwGradshape(dof,5);
            //tSh(3,3) = SkwGradshape(dof,15);

            // SkwGrad SKwGrad transformation
            //Mult(tSh, J, trShTemp);
            //Mult(Jtr, trShTemp, tSh);
             
            // Curl Curl Transformation
             Mult(tSh, invJ, trShTemp);
             Mult(invJtr, trShTemp, tSh);
            double s_factor = 1.0;
            SkwGradshape_dFt(dof,0) = s_factor * tSh(0,1);
            SkwGradshape_dFt(dof,1) = s_factor * tSh(0,2);
            SkwGradshape_dFt(dof,2) = s_factor * tSh(0,3);
            SkwGradshape_dFt(dof,3) = s_factor * tSh(1,2);
            SkwGradshape_dFt(dof,4) = s_factor * tSh(1,3);
            SkwGradshape_dFt(dof,5) = s_factor * tSh(2,3);
      ...
      {
          mfem_error("SkwGrad : SkwGrad integrator not valid for current Dim");
      }
...
//         Mult(curlshape_dFt, M, curlshape);
//         AddMultABt(curlshape, curlshape_dFt, elmat);
          mfem_error("Coeficent type not supported");
