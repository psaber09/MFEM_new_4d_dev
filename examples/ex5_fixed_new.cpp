...
int main(int argc, char *argv[])
{
    ...
    for (int r : ref_levels)
    {
        ...
        H1_FECollection h1_vec_coll(order+1, dim);
        H1_FECollection h1_scalar_coll(order, dim);
        FiniteElementSpace R_space(&mesh, &h1_vec_coll, dim, Ordering::byVDIM);
        FiniteElementSpace W_space(&mesh, &h1_scalar_coll);
