...
int main(int argc, char *argv[])
{
   ...
   std::cout << "DivSkw FECollection Used" << std::endl;
...
//         << fespace->GetTrueVSize() << endl;
   ...
   //     later using GLVis: "glvis -m refined.mesh -g sol.gf".
   {
      ...
      mesh->Print(mesh_ofs);
      ofstream sol_ofs("sol_DivSkw.gf");
      sol_ofs.precision(8);
      x.Save(sol_ofs);
   }
...
//       f(2,3) =  sf*(2.0 + 2.0  * M_PI*M_PI)*c0*c1*s2*s3;
//       f(1,3) =  sf*(2.0 + 0.0  * M_PI*M_PI)*c0*s1*c2*s3;
//       f(1,2) =  sf*(2.0 + 2.0  * M_PI*M_PI)*c0*s1*s2*c3;
//       f(0,3) =  sf*(2.0 - 2.0  * M_PI*M_PI)*s0*c1*c2*s3;
//       f(0,2) =  sf*(2.0 + 0.0  * M_PI*M_PI)*s0*c1*s2*c3;
//       f(0,1) =  sf*(2.0 + 2.0  * M_PI*M_PI)*s0*s1*c2*c3;
