     ...
     M(M_), K(K_), b(b_), z(height)
{
   ...
   if (M.GetAssemblyLevel() == AssemblyLevel::LEGACY)
   {
      ...
      const int dofs_per_elem = M.FESpace()->GetFE(0)->GetDof();
      const int vdim          = M.FESpace()->GetVDim();
      const int block_size    = vdim * dofs_per_elem;

      auto *blkilu = new BlockILU(block_size, BlockILU::Reordering::MINIMUM_DISCARDED_FILL);
      blkilu->SetOperator(M.SpMat());
      M_prec = blkilu;
